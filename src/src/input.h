...
public:
	...
	This is used with the the input managers iterface.

	Note: this codes is an altered version of SDL_scancode.h file.

	so I'm refereing them here. Thanks SDL for some really amazing code :),
	also note you can also pass SDL_scancodes to isKeyPress and isKeyHit, if
	you want to, but just use KB_* codes because its shorter to type in.
	*/
	enum Keyboard
	{
		KB_UNKNOWN = 0,
...
private:
	...
	// Keyboard
	// Keyboard States
	InputState keyStates[Input::KB_SIZE];
	// Mouse
	// Mouse States
	InputState mouseStates[Input::MB_SIZE];
	// Mouse Positions
	Uint32 mx;
	Uint32 my;
...
public:
	...
	// Keyboard 
	bool isKeyHit(Keyboard k);

	bool isKeyPress(Keyboard k);

	// Mouse
	bool isMouseButtonHit(MouseButton mb);

	bool isMouseButtonPress(MouseButton mb);

	void getMousePosition(glm::ivec2& mc);
